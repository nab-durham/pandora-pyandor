...
andor_lib_name = ctypes.util.find_library("andor")
assert not andor_lib_name is None, "Can't find Andor library"
print("Andor library name is "+andor_lib_name)
andor = ctypes.cdll.LoadLibrary(andor_lib_name)

## initialise
#
ret_code=andor.Initialize("/usr/local/etc/andor".encode('ascii')) # obviously Oxford educated...hoho
assert ret_code==AndorCodes.DRV_SUCCESS, "Got "+str(ret_code)+"/"+str(AndorCodes.values[ret_code])
print("Initialised SDK")

## camera number information
#
lNumCameras=ctypes.c_long()
ret_code=andor.GetAvailableCameras(ctypes.byref(lNumCameras))
if ret_code==AndorCodes.DRV_SUCCESS:
    print("Found "+str(lNumCameras.value)+" cameras")
else:
    print("Return code was "+AndorValues[ret_code])

## choose camera
#
lCameraHandle=ctypes.c_long()
ret_code=andor.GetCameraHandle(ctypes.c_int(0), ctypes.byref(lCameraHandle))
assert ret_code==AndorCodes.DRV_SUCCESS, "Got "+str(ret_code)+"/"+str(AndorCodes.values[ret_code])
ret_code=andor.SetCurrentCamera(lCameraHandle)
assert ret_code==AndorCodes.DRV_SUCCESS, "Got "+str(ret_code)+"/"+str(AndorCodes.values[ret_code])
print("Set current camera to be #0")
...
#    if(error==DRV_SUCCESS) error=GetDetector(&width, &height); ** READ ONLY **
#    if(error==DRV_SUCCESS) error=SetShutter(1,0,50,50); ** SOME DEFAULT, INVESTIGATE **
#    if(error==DRV_SUCCESS) error=SetTriggerMode(trigMode); ** IMPORTANT **
#    if(error==DRV_SUCCESS) error=SetAcquisitionMode(AcqMode); ** IMPORTANT **
#    if(error==DRV_SUCCESS) error=SetReadMode(ReadMode); ** IMPORTANT **
#    if(error==DRV_SUCCESS) error=SetExposureTime(expoTime); ** IMPORTANT ** 
#    if(error==DRV_SUCCESS) error=SetAccumulationCycleTime(accTime); ** NO **
#    if(error==DRV_SUCCESS) error=SetNumberAccumulations(numAccs); ** NO **
#    if(error==DRV_SUCCESS) error=SetKineticCycleTime(kinTime); ** NO **
#    if(error==DRV_SUCCESS) error=SetNumberKinetics(numKins); ** NO **
#    if(error==DRV_SUCCESS) error=SetMultiTrack(numTracks,trackHeight,trackOffset, &trackBottom, &trackGap); ** NO **
#    if(error==DRV_SUCCESS) SetReadMode(2);  ** CHECK WHAT THIS DOES **
#    if(error==DRV_SUCCESS) error=SetRandomTracks(numTracks, randomTracks); ** PROBABLY DON'T DO THIS **
#    if(error==DRV_SUCCESS) SetReadMode(ReadMode); ** WANT CONTINUOUS/VIDEO MODE **
...
#    if(error==DRV_SUCCESS) error=SetHSSpeed(0, 0); ** CHECK WHAT THE PARAMS ARE FOR THIS **
...
ret_code=andor.GetDetector(ctypes.byref(width),ctypes.byref(height))
assert ret_code==AndorCodes.DRV_SUCCESS, "Got "+str(ret_code)+"/"+str(AndorCodes.values[ret_code])
print("Size of detector is :"+str(height)+"x"+str(width))

## Shutdown
#
ret_code=andor.ShutDown()
assert ret_code==AndorCodes.DRV_SUCCESS, "Got "+str(ret_code)+"/"+str(AndorCodes.values[ret_code])
